...
bool isPromise(move_data md){
    ...
    if(cur_wall_pass_cnt <= K){
        ...
        // 더 적게 벽 통과로 해당 위치에 도달하는 최소 이동 횟수를 가져온다.
        ...
        // 1. 현재 이용한 벽통과 횟수로 해당 칸을 방문한 적이 없어야 한다.
        // 2. 더 적은 벽 통과로 이동한 횟수보다 이동 횟수가 적어야 한다.
        if(!visited_data[cur_r][cur_c][cur_wall_pass_cnt] && less_use_less_move > md.move_cnt){
            return true;
        }
    }
        ...
        for(int i = 0; i < DIR; i++){
            ...
            if(inRange(nr, nc)){
                ...
                if(isPromise(next_move_data)){
                    ...
                    if(next_move_data.r == N && next_move_data.c == M){
                        return next_move_data.move_cnt;
                    }
...
int main() {
    ...
    visited_data.resize(N+1, vector<vector<int>>(M+1, vector<int>(K+1)));
